...
*/
  ...
  Future google2({BuildContext context}) async {
    try {
      SocialLogin().googleLogin().then(
            (value) => {
              if (value != null &&
                  value != "" &&
                      ...
                      .then(
                        (googdata) => {
                          if (googdata.status == 1 &&
                            ...
                            {
                              userrepo
                                  .getUser(email: value.googleProfile.email)
                                  .then(
                                    (profile) => {
                                      debugPrint(profile.docs[0].userName),
                                      if (profile.status == 1)
                                        {
                                          setState(() {
                                            isloading = false;
                                            ...
                                            Navigator.pushReplacement(context,
                                                MaterialPageRoute(
                                                    builder: (context) {
                                              return OTPScreen(
                                                profile.docs[0].userMobile,
                                                "Register",
                                                email:
                                                    profile.docs[0].userEmail,
                                              );
                                            }));
                                          })
                                        }
  ...
  // Future fbLogin2({BuildContext context}) async {
  //   try {
  //     SocialLogin().fbLogin(context: context).then(
  //           (value) => {
  //             print('up' + value.profile['email']),
  //             if (value != null && value != "")
  //               {
  //                 print('down' + value.profile['email']),
  //                 setState(() {
  //                   isloading = true;
  //                 }),
  //                 fbrepo
